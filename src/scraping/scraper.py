...
def session(path_driver, url):
    ...
    options.add_argument("--allow-running-insecure-content") # Permite contenido http en páginas https.
    options.add_argument("user-agent=[user-agent string]") # Establece un user agent customizado.
    options.add_argument("executable_path=" + path_driver) # Ruta al ejecutable del chromedriver.
    ...
    options.add_experimental_option("useAutomationExtension", False) # Evita la instalación automática de esta extensión. Hace que Chrome sea invisible en sitios que bloquean la automatización. El contra es perder acceso a algunas APIs útiles para automatización.
